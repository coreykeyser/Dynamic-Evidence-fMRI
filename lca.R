# 
# 
# rlca_v1=function(n.items, max.time, startx, drift, K, L, nu, eta, thresh, dt, tau, t0){
#   #   // Params:
#   #   // -------
...
# determine the change in x
temp=rnorm(1,0,eta)
x[t,i] = x[t-1,i] + (drift[t,i] - (K)*(x[t-1,i]) - (L)*lx[i] - nu/(n.items-1)*sum(drift[t,-i]) )*(dt_tau) + (temp*sqrt_dt_tau)
...
rand.sample = function(x){
  out[1] = sample(x,1)
  for(i in 2:length(x)){
    idx = which(x == out[i-1])
    if(x[idx] == min(x)){
      idx = c(idx,idx+1)
      out[i] = sample(x[idx],1)
    } 
    if(x[idx] == max(x)){
      idx = c(idx,idx-1)
      out[i] = sample(x[idx],1)
    } 
    if(idx != min(x) & idx != max(x)){
