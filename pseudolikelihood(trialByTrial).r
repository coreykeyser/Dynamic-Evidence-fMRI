    ...
    for(i in 1:numtrials){
      for(i in 1:n.obs){
        temp=rlcca(n.items = true$n.items[1], max.time = true$maxtime[1], startx = true$startx, 
                   drift = drift[(((i-1)*60)+1):(i*60),], K = params['K'], L = params['L'], eta = true$eta[1], dt = true$dt[1],
      ...
      logprobtrial[i]=(pseudolikelihood(data.mx=sim.data,
                              obs.data=obs.data))
      
    }
    if(is.finite(logprobtrial)){return(sum(logprobtrial))}
    else{return(-Inf)}
